/*
Purpose: Ensures event UID assignment is deterministic.

What this tests: UIDs generated by a sender LP are stable (independent of rank) and can
be used reliably for cancellation/anti matching.
*/
...
int main()
{
    ...
    // UIDs are per-LP 64-bit monotonically increasing counters. Sender is LP 1.
    const warpsim::EventUid expectedFirst = 1ULL;
    const warpsim::EventUid expectedSecond = 2ULL;
